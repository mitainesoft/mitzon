            ...
            if time.time() > (self.weather_manager_start_time+60):
                if cherrypy.engine.state == cherrypy.engine.states.STARTED:
                    ...
                    if (time.time() > (self.cherryweb_server_last_run_time + 120) ):
                        ...
                        status_text = self.alarm_mgr_handler.addAlert("SW001", "WEATHER", "cherryweb_server_last_run_time" )
            ...
            try:
                async with session.get(meteo_url) as resp:
                    ...
                    if resp.status != 200: # 200 is normal response
                        logstr = self.meteo_json + " Status:" + str(resp.status) + " Content-type:", resp.headers['content-type']
                        log.error(logstr)
                        status_text = self.alarm_mgr_handler.addAlert("SW001", "WEATHER" ,logstr)
                        log.error(status_text)
                ...
                log.error("Get Weather Error, check url:"+self.meteo_url_json)
                traceback.print_exc()
